# Urban Shopfronts — agent notes

Next.js 16 App Router · TypeScript · Tailwind v4 · Prisma + PostgreSQL · Vercel
(auto-deploys from `main`).

**Live:** https://www.urbanshopfronts.co.uk
**Theme:** Dark, gold accent
**Phone:** 07471 043827 · **Base:** West Midlands (Smethwick)
**Instagram:** instagram.com/urbanshopfrontsuk · 10+ years trade experience

**Positioning:** Fitting, repair and commercial fit-out — including office partitioning, which neither sibling offers.
...
- **`cityPages?: boolean` on the `Service` interface** (`data/services.ts`)
  gates the city cross-product. Defaults to `true`. `aluminium-partitions` and
  `glass-partitions` set it to `false` — they are hub pages only, because
  office partitioning is specified from drawings, not from
  "partitions in <town>" searches, and this site already submits 658 URLs
  against one external backlink.

  Every consumer of the matrix must read **`cityPageServices`**, not
  `services`: `generateStaticParams`, the service grid on `/areas/[city]`,
  the "Across the UK" grid on `/services/[slug]`, IndexNow, and
  `app/sitemap.ts` (where the two slugs join `servicePages` but not
  `serviceCityPages`). Adding a new service without checking this will
  silently create 41 city pages.
- Adding a normal service to `data/services.ts` **does** create 41 city pages
  automatically. That is usually not what you want on this site.
- Title qualifier is `' | Fit & Repair'`.
- Best performer of the three (position ~25.7) and the healthiest. The
  portfolio review's conclusion: if one shopfront site is going to be the
  survivor, the data says this one. Treat changes here conservatively.
- Still carries the 574-page cross-product. Not retired.

## Current state

- 16 services in `data/services.ts`, 41 cities in `data/cities.ts`
