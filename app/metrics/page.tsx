...
export default function MetricsPage() {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('urban_metrics_key');
    if (saved) {
      setApiKey(saved);
  ...
  const fetchMetrics = useCallback(async (key: string, period: number) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        sessionStorage.removeItem('urban_metrics_key');
        return;
      }
      ...
      setData(json);
      setAuthenticated(true);
      sessionStorage.setItem('urban_metrics_key', key);
    } catch {
    ...
    return (
      <div className="min-h-screen bg-grey-50 flex items-center justify-center px-4">
        <form onSubmit={handleLogin} className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
          ...
          <h1 className="font-heading font-bold text-xl text-navy text-center mb-1">Urban Metrics</h1>
      ...
      <header className="bg-navy border-b border-navy-light sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="font-heading font-bold text-gold text-lg">Urban Metrics</span>
            {loading && (
