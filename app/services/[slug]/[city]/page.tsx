...
function getCityLocalContent(cityName: string, region: string, serviceName: string, areas: string[], postcodeAreas: string[]): { heading: string; paragraphs: string[] } {
  ...
  const headingVariants = [
    `Why ${cityName} Businesses Choose Urban Shopfronts`,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Affordable ${service.name.toLowerCase()} in ${city.name} — competitive prices, free site survey & no-obligation quotes. Covering ${topAreas} and surrounding areas. Same-day response available. Call 07471 043827.`,
